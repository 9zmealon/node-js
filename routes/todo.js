...
router.post('/', [
    ...
    if (!errors.isEmpty()) {
        ...
        connection.query('insert into todo set ?', insertObj, function (err, result) {
            if (err) throw err;
            res.redirect(301, '/todo/' + result.insertId);
            // res.json({
            //     id : result.insertId,
...
router.put('/:id', [
    ...
    var query = connection.query('update todo set ? where id = ?', [updateObj, id], function (err, result) {
        if (err) throw err;
        res.redirect(303, '/todo/' + id); //303 for put method.
        // res.json({
        //     ...result,
...
router.get('/:status', function (req, res, next) {
    ...
    connection.query('select * from todo where status = ?', status, function (err, result, fields) {
        if (err) {
            throw err;
        }
        res.send(result);
